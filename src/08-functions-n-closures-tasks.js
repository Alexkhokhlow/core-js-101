...
function getPolynom(...arg) {
  return (x) => arg.reduce((acc, item, index) => acc + item * x ** (arg.length - index - 1), 0);
}
...
function logger(func, logFunc) {
  return (...x) => {
    const item = Array.from(JSON.stringify(x));
    item.splice(0, 1);
    item.splice(item.length - 1, 1);
    logFunc(`${func.name}(${item.join('')}) starts`);
    const result = func(...x);
    logFunc(`${func.name}(${item.join('')}) ends`);
    return result;
  };
}
